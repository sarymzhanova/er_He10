...
void ERNDDigitizer::Exec(Option_t* opt)
{
  ...
  for (const auto &itCrystall : pointsByCrystall){
    ...
    time = gRandom->Gaus(time, timeSigma);      
      ...
      switch (parentTrackID)
      {
         case 3:         
            ...
            time_n1 = point->GetTime();
            break;
         case 4:
            edep_n2+=point->GetEnergyLoss();
            time_n2 = point->GetTime();
            break;
         default:
    ...
    */
    ...
    AddDigi(itCrystall.first,edep, edep_n1, edep_n2, edep_mis, parentTrackID, ly, time,time_n1,time_n2,time_mis, neutronProb,x_in,y_in,z_in);	//sonya:new variables x_in,y_in,z_in - enter point of neutron (proton in fact) in stilben
  }
