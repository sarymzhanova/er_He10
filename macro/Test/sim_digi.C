...
//----------------------------------
  ...
  TString appendName = "sim_digi_8_1nNDSteel.root";
...
//  generator->SetPSigmaOverP(0);				//sigmaPOverP	percentage of ion momentum. 
  generator->SetPSigmaOverP(0.04);
...
//  generator->SetBoxXYZ(0, 0, 0., 0., beamStartPosition);	//задает ограничение размытия пятна пучка на мишени
  ...
  generator->SpreadingOnTarget(); 				//Sets spreading of x and y coordinates on target (where z-position is zero) and 
  								...
  								//By default spreading of x and y coordinates implements on set z-coordinate. 
...
//  targetDecay->SetHe10Exitation(0.0021, 0.002, 1);			//[GeV]
  ...
  beamDetDigitizer->SetToFTimeSigma(0.1); //in ns = 100 ps 
