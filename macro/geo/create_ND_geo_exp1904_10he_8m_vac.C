void create_ND_geo_exp1904_10he_8m_vac()
{
  TString erPath = gSystem->Getenv("VMCWORKDIR");

  // Output paths
  TString outGeoFilenameRoot = erPath + "/geometry/ND.geo.exp1904.10he.8m.vac.root";
  ...
  if ( ! mvacuum ) Fatal("SOS", "FairMedium vacuum not found");
  geoBuild->createMedium(mvacuum);
  TGeoMedium* pvacuum = geoM->GetMedium("vacuum");
  if ( ! pvacuum ) Fatal("SOS", "Medium vacuum not found");

  // General dimensions
  Double_t R_min_Crystal = 0.0; // cm
  Double_t R_max_Crystal = 4.0; // cm
//  Double_t HeightZ_Crystal = 50.0; // cm
  Double_t HeightZ_Crystal = 5.0; // cm
  Double_t R_min_Shell = 0.0; // cm
  Double_t R_max_Shell = 4.2; // cm
//  Double_t HeightZ_Shell = 50.2; // cm
  Double_t HeightZ_Shell = 5.2; // cm
  Double_t R_min_Housing = 0.0; // cm
  Double_t R_max_Housing = 4.8; // cm
//  Double_t HeightZ_Housing = 50.4; // cm
  Double_t HeightZ_Housing = 5.4; // cm

  // Shapes
//  TGeoBBox* PlaneShape = new TGeoBBox("PlaneShape",400./2., 400./2., 460./2.);
  ...
  // Volumes
  TGeoVolume* AirBox = new TGeoVolume("NDAirBox", PlaneShape, pvacuum);
...
float xyz[45][3] = {  
	...
	-4*R,	-2*h,	7925	,	//6
	-6*R,	-2*h,	7900	,	//7
	-8*R,	-2*h,	7865	,	//8	__________________________
	9*R,	-h,	7865	,	//9
	7*R,	-h,	7900	,	//10
	5*R,	-h,	7925	,	//11
	3*R,	-h,	7939.9	,	//12
	...
	-3*R,	-h,	7925	,	//15
	-5*R,	-h,	7900	,	//16
	-7*R,	-h,	7865	,	//17	__________________________
	8*R,	0.0,	7865	,	//18
	6*R,	0.0,	7900	,	//19
	4*R,	0.0,	7925	,	//20
	2*R,	0.0,	7939.9	,	//21
	...
	-4*R,	0.0,	7925	,	//24
	-6*R,	0.0,	7900	,	//25
	-8*R,	0.0,	7865	,	//26	__________________________
	9*R,	h,	7865	,	//27
	7*R,	h,	7900	,	//28
	5*R,	h,	7925	,	//29
	3*R,	h,	7939.9	,	//30
	...
	-3*R,	h,	7925	,	//33
	-5*R,	h,	7900	,	//34
	-7*R,	h,	7865	,	//35	__________________________
	8*R,	2*h,	7865	,	//36
	6*R,	2*h,	7900	,	//37
	4*R,	2*h,	7925	,	//38
	2*R,	2*h,	7939.9	,	//39
